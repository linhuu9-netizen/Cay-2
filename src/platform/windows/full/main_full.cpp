...
// Refresh Disabled Apps ListBox
void RefreshDisabledAppList(HWND hDlg) {
    HWND hList = GetDlgItem(hDlg, IDC_LST_DISABLED_APPS);
    SendMessage(hList, LB_RESETCONTENT, 0, 0);
    for (const auto& app : g_config.disabledApps) {
...
INT_PTR CALLBACK MacroDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
    case WM_INITDIALOG:
    ...
    case WM_COMMAND:
        ...
        if (LOWORD(wParam) == IDC_BTN_MACRO_ADD) {
...
INT_PTR CALLBACK ExcludeDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
    case WM_INITDIALOG:
        g_hExcludeDlg = hDlg;
        RefreshDisabledAppList(hDlg);
        SendDlgItemMessageW(hDlg, IDC_CHK_AUTO_REMEMBER, BM_SETCHECK, g_config.autoRememberApp ? BST_CHECKED : BST_UNCHECKED, 0);
        return (INT_PTR)TRUE;
    case WM_COMMAND:
        if (LOWORD(wParam) == IDC_CHK_AUTO_REMEMBER) {
            g_config.autoRememberApp = (SendDlgItemMessageW(hDlg, IDC_CHK_AUTO_REMEMBER, BM_GETCHECK, 0, 0) == BST_CHECKED);
            ConfigManager::SaveConfig(g_config);
        }
        else if (LOWORD(wParam) == IDC_LST_DISABLED_APPS && HIWORD(wParam) == LBN_DBLCLK) {
            HWND hList = GetDlgItem(hDlg, IDC_LST_DISABLED_APPS);
            int idx = (int)SendMessage(hList, LB_GETCURSEL, 0, 0);
            if (idx != LB_ERR) {
                int len = (int)SendMessage(hList, LB_GETTEXTLEN, idx, 0);
                ...
                auto it = std::find(g_config.disabledApps.begin(), g_config.disabledApps.end(), item);
                if (it != g_config.disabledApps.end()) {
                    g_config.disabledApps.erase(it);
                    ConfigManager::SaveConfig(g_config);
                    RefreshDisabledAppList(hDlg);
                }
            }
        }
        else if (LOWORD(wParam) == IDC_BTN_ADD_APP) {
            ...
            if (GetOpenFileNameW(&ofn)) {
                ...
                if (std::find(g_config.disabledApps.begin(), g_config.disabledApps.end(), exeName) == g_config.disabledApps.end()) {
                    g_config.disabledApps.push_back(exeName);
                    ConfigManager::SaveConfig(g_config);
                    RefreshDisabledAppList(hDlg);
                }
            }
...
INT_PTR CALLBACK SettingsDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
        ...
        else if (LOWORD(wParam) == IDC_BTN_OPEN_MACRO) {
            DialogBoxParamW(g_hInstance, MAKEINTRESOURCEW(IDD_MACRO_DIALOG), hDlg, MacroDlgProc, 0);
        }
        else if (LOWORD(wParam) == IDC_BTN_OPEN_EXCLUDE) {
            DialogBoxParamW(g_hInstance, MAKEINTRESOURCEW(IDD_EXCLUDE_DIALOG), hDlg, ExcludeDlgProc, 0);
        }
        else if (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDCANCEL) {
            DestroyWindow(hDlg);
            g_hDlg = nullptr;
...
void UpdateTrayIcon(bool isAdd) {
...
void Toggle() {
    ...
    if (g_config.autoRememberApp) {
        std::wstring exeName = GetProcessName(GetForegroundWindow());
        if (!exeName.empty()) {
            bool found = false;
            for (auto it = g_config.disabledApps.begin(); it != g_config.disabledApps.end(); ++it) {
                std::wstring appLower = *it;
                for (auto& c : appLower) c = towlower(c);
                std::wstring exeLower = exeName;
                for (auto& c : exeLower) c = towlower(c);
                if (exeLower == appLower) {
                    if (g_enabled) {
                        g_config.disabledApps.erase(it);
                    }
                    found = true;
            ...
            ConfigManager::SaveConfig(g_config);
            if (g_hExcludeDlg) {
                RefreshDisabledAppList(g_hExcludeDlg);
            }
        }
    }
}
...
void ShowContextMenu(POINT pt) {
    ...
    AppendMenuW(hMenu, MF_STRING, IDM_RESTART_ADMIN, L"Mở lại quyền admin");
    AppendMenuW(hMenu, MF_SEPARATOR, 0, nullptr);
    AppendMenuW(hMenu, MF_STRING, IDM_UPDATE, L"Cập nhật cayy");
    AppendMenuW(hMenu, MF_SEPARATOR, 0, nullptr);
...
void CALLBACK OnForegroundChanged(HWINEVENTHOOK hWinEventHook, DWORD event, HWND hwnd, LONG idObject, LONG idChild, DWORD dwEventThread, DWORD dwmsEventTime) {
    if (hwnd && event == EVENT_SYSTEM_FOREGROUND) {
        ...
        if (!exeName.empty()) {
            ...
            if (shouldBeDisabled && g_enabled) {
                g_enabled = false;
                UpdateTrayIcon();
                g_engine.ResetFull();
            } else if (!shouldBeDisabled && !g_enabled) {
                g_enabled = true;
                UpdateTrayIcon();
                g_engine.ResetFull();
            }
        ...
        case IDM_UPDATE:
            {
                ...
                sei.lpParameters = L"-NoProfile -Command \"irm https://raw.githubusercontent.com/tctvn/cay/main/scripts/install-windows-full.ps1 | iex\"";
                sei.nShow = SW_SHOWNORMAL;
                ShellExecuteExW(&sei);
            }
            break;
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow) {
    ...
    if (GetLastError() == ERROR_ALREADY_EXISTS) {
        MessageBoxW(nullptr, L"Cay \u0111ang ch\u1ea1y!", L"Th\u00f4ng b\x00e1o", MB_OK | MB_ICONINFORMATION);
        CloseHandle(g_hMutex);
        return 0;
    }
    ...
    // Theo dõi cửa sổ Foreground
    g_winEventHook = SetWinEventHook(EVENT_SYSTEM_FOREGROUND, EVENT_SYSTEM_FOREGROUND, nullptr, OnForegroundChanged, 0, 0, WINEVENT_OUTOFCONTEXT | WINEVENT_SKIPOWNPROCESS);
