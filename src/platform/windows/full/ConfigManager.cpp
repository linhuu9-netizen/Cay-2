...
ConfigData ConfigManager::LoadConfig() {
    ...
    // Parse đơn giản
    size_t pos = 0;
    int section = 0; // 1 = Hotkey, 2 = Macro, 3 = General, 4 = DisabledApps
    while (pos < text.length()) {
        ...
        if (line == L"[Hotkeys]") section = 1;
        ...
        else if (line == L"[General]") section = 3;
        else if (line == L"[DisabledApps]") section = 4;
        else if (!line.empty()) {
            ...
            if (eq != std::wstring::npos) {
                ...
                if (section == 1) {
                    ...
                    if (key == L"AutoRememberApp") data.autoRememberApp = (val == L"1");
                }
            } else if (section == 4) {
                data.disabledApps.push_back(line);
            }
        }
...
void ConfigManager::SaveConfig(const ConfigData& data) {
    ...
    text += L"\r\n[General]\r\n";
    text += L"AutoRememberApp=" + std::wstring(data.autoRememberApp ? L"1" : L"0") + L"\r\n";
    
    text += L"\r\n[DisabledApps]\r\n";
    for (const auto& app : data.disabledApps) {
        text += app + L"\r\n";
    }
